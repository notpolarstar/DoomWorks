...
WAD_ABS := $(abspath $(WAD))
EXTERNAL_DATA_FILE_ABS := $(if $(strip $(EXTERNAL_DATA_FILE)),$(abspath $(EXTERNAL_DATA_FILE)),)
...
ifneq ($(strip $(EXTERNAL_DATA_FILE_ABS)),)
	$(SIM_BIN) --nwb $(NWB_FILE) --nwb-external-data $(EXTERNAL_DATA_FILE_ABS)
else
