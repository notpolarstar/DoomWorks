      ...
      if (!--door->topcountdown)  // downcount and check
      {
        switch(door->type)
        {
          ...
          case genBlazeRaise:
            ...
            S_StartSound2(&g_dummy_soundorg,sfx_bdcls);
            break;

          case normal:
          case genRaise:
            door->direction = -1; // time to go back down
            S_StartSound2(&g_dummy_soundorg,sfx_dorcls);
            break;

          case close30ThenOpen:
          case genCdO:
            door->direction = 1;  // time to go back up
            S_StartSound2(&g_dummy_soundorg,sfx_doropn);
            break;

          case genBlazeCdO:
            door->direction = 1;  // time to go back up
            S_StartSound2(&g_dummy_soundorg,sfx_bdopn);
            break;
