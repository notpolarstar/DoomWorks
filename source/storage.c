...
int extapp_fileList(const char ** filename, int maxrecord, const char * extension) {
  ...
  while ((currentRecord < maxrecord) && offset < endAddress) {
    ...
    // Corrupted records must not make us walk out of storage.
    if (size < 3 || (offset + size) > endAddress) {
      break;
    }
...
int extapp_fileListWithExtension(const char ** filename, int maxrecord, const char * extension_to_match) {
  ...
  while ((currentRecord < maxrecord) && offset < endAddress) {
    ...
    if (dot != NULL && strcmp(dot + 1, extension_to_match) == 0) {
      filename[currentRecord] = name;
      currentRecord++;
    }
...
#endif
  uint32_t storageAddress = extapp_address();
  const uint32_t storageSize = extapp_size();
  const uint32_t preserveBytes = 1024;
  uint32_t wipeOffset;

  if (!extapp_isValid((const uint32_t *)storageAddress)) {
    return false;
  }

  wipeOffset = (storageSize > preserveBytes) ? preserveBytes : 4;
  if (wipeOffset >= storageSize) {
    return true;
  }

  memset((void *)((uintptr_t)storageAddress + wipeOffset), 0, storageSize - wipeOffset);
  return true;
}
