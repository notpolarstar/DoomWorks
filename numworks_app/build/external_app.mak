...
ifeq ($(PLATFORM),device)
  ...
  LTO = 0
  ifeq ($(HOST),linux)
    SIMULATOR ?= ../epsilon.bin
  else ifeq ($(HOST),macos)
    SIMULATOR ?= ../epsilon.app/Contents/MacOS/Epsilon
  else ifeq ($(HOST),windows)
    SIMULATOR ?= ../epsilon.exe
  else
...
$(BUILD_DIR)/$(APP_NAME).nwa: $(call object_for,$(SOURCES)) $(BUILD_DIR)/icon.o | $(BUILD_DIR)
	@echo "LD      $@"
...
$(addprefix $(BUILD_DIR)/,%.o): %.cpp | $(BUILD_DIR) ; @echo "CC      $<" ; mkdir -p $(dir $@) && $(CXX) $(CXXFLAGS) -c $< -o $@

$(addprefix $(BUILD_DIR)/,%.o): %.c | $(BUILD_DIR) ; @echo "CC      $<" ; mkdir -p $(dir $@) && $(CC) $(CFLAGS) -c $< -o $@

$(BUILD_DIR)/icon.o: $(APP_ICON) | $(BUILD_DIR)
	@echo "ICON    $<"
	$(Q) $(NWLINK) png-icon-o $< $@
